...
def add_new_inventory_item(item_data: dict):
    ...
    try:
        ...
        response = make_api_request("POST", "create-item/", headers={"Content-Type": "application/json"}, json=payload)
...
def update_existing_inventory(item_data: dict):
    ...
    try:
        ...
        response = make_api_request("PUT", f"update/{inventory_id}/", headers={"Content-Type": "application/json"}, json=payload)
        handle_validation_response(response)
        return response.json()
